...
test("real privacy pool: LockTicket supply two survives take and both maker settlement pulls", {
  timeout: TX_TIMEOUT,
}, async () => {
  buildArtifacts();
  ...
  try {
    ...
    const maker = makePrivacy(env, env.alice, "app20-lock-ticket-maker");
    const taker = makePrivacy(env, env.bob, "app20-lock-ticket-taker");
    await shield(
      devnet,
      env,
      env.alice,
    ...
    const ticketNotesAfterLock = await notesFor(maker.transfers, devnet, [
      lockTicket,
    ]);
    assert.ok(
    ...
    const fillsDigest = ec.starkCurve.poseidonHashMany([
      BigInt(LOCK_ID),
      TAKEN_A,
    ]);
    ...
    const takeSignature = ec.starkCurve.sign(
    ...
    const ticketNotesAfterProceeds = await notesFor(maker.transfers, devnet, [
      lockTicket,
      env.eth,
    ]);
    assert.ok(
