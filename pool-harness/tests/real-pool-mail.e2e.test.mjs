...
function parseMessageEvent(event) {
	assert.equal(event.keys.length, 2, "MessagePosted must have selector and index keys");
...
function makeAlicePrivacy(env) {
	...
	const prover = new CorePrivateTransfersProver({
		...
		// an address so the same production prover can support them when requested.
...
test(
	...
	async () => {
		...
		const strk20 = await import(
			pathToFileURL(join(BUILD_DIR, "strk20.js")).href
		);
		...
		try {
			...
			const actionId = strk20.computeActionId(
				"mail",
				"real-pool-nullifier-e2e",
			);
			assert.notEqual(BigInt(actionId), 0n, "replay-protected action id must be non-zero");
			...
			assert.equal(
				(
					await mail.scanAndDecrypt(strangerMail.privateKey, [eventRecord])
				).length,
				0,
			...
			const openNoteDepositSelector = hash.getSelectorFromName(
				"OpenNoteDeposited",
			);
