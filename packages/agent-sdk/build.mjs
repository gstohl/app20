...
await build({ absWorkingDir: root, entryPoints: ['packages/agent-sdk/src/confidential.ts'], outfile: 'packages/agent-sdk/dist/confidential.js', bundle: true, platform: 'node', format: 'esm', target: 'node24', external: ['starknet'], banner, legalComments: 'inline' });
...
try {
  ...
  for (const name of ['index', 'types', 'privacy', 'confidential', 'confidential-protocol', 'confidential-journal']) {
