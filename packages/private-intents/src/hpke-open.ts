...
import { canonicalRfqTransportAad, PRIVATE_RFQ_DOMAIN } from "#protocol";
import { RFQ_HPKE_INFO, createRfqHpkeSuite, decodeBase64url, unpadRfqPlaintext } from "#hpke";

const encoder = new TextEncoder();

function parsePrivateRfq(canonical: string): PrivateRfqV1 {
  let value: unknown;
  try { value = JSON.parse(canonical); } catch { throw new Error("RFQ HPKE plaintext is invalid."); }
  if (!value || typeof value !== "object") throw new Error("RFQ HPKE plaintext is invalid.");
...
export function createRfqEnvelopeOpener(
  ...
  return {
    async open(envelope, transportKey) {
      try {
        const recipientKey = await resolvePrivateKey(transportKey.keyId);
        ...
        return parsePrivateRfq(unpadRfqPlaintext(new Uint8Array(plaintext)));
      } catch {
        throw new Error("RFQ envelope HPKE authentication or decryption failed.");
      }
    },
  };
}
