...
- V3 lock records retain exact schedule, maximum collateral, taken A/B, quote digest, and maker settlement hashes. Every five-second localnet scan finalizes both ticket-authorized sides after expiry, omitting an output note when a side is zero; the Cairo lock closes only after both units are consumed. Read failures move a lock to durable `reconcile-pending` while retaining its prior effective state; submitted settlement uncertainty moves it to `settlement-unknown` with the exact action, attempt, and transaction hash when available. Both states reconcile without resubmission using receipt plus `get_lock` reads, exponential backoff capped at 60 seconds, and a 20-failure bound.
