...
- **P0-33 — No user mailbox key rotation, revocation, or compromise recovery.** The mailbox recovery phrase deterministically derives the Mail *signing* key (`src/lib/mail-auth.ts:45-52,80-83`), which signs payment requests that `/pay` presents as `MAIL SIGNATURE VERIFIED`, yet onboarding warns only that the phrase permits *reading* mail (`src/components/mail/Onboard.tsx:468-470`). Anyone holding it can therefore both decrypt future correspondence and mint convincing signed payment requests indefinitely. Refusing to overwrite a registered key is correct fail-closed behaviour; what is absent is a key epoch, revocation path, correspondent transition, old-mail recovery, and an onboarding warning that states the signing authority.
