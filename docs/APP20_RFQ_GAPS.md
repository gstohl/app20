...
- **P0-20 — No constructible configured-chain verifier exists.** `src/lib/settlement-receipt-chain.ts` exposes a nominal capability but intentionally has no public constructor/composition root and always refuses execution.
...
- **P1-15 — Surface separation.** Public send, cross-chain dry review, and Privy recovery currently share the RFQ page. They need deliberate secondary placement or separate routes so wallet custody and RFQ execution are not conflated.
