...
Updated September 8, 2026. A real STARK proof settled the jointly authorized escrow on mainnet: both agreed outputs arrived as encrypted notes, the escrow became settled and empty, and no public trade value actions or OPEN outputs were created. Receipt, trace, deployed class pins and encrypted balance deltas are verified in [the durable evidence](../deployments/mainnet/confidential-settlement-2026-09-08.json). One operator controlled both wallets and funded both legs using the Node SDK. The browser wallet integration is implemented; native Ready end-to-end acceptance, mainnet refunds and independent review remain unverified.
...
The local control service binds to loopback, rejects cross-origin writes and serializes mutations. Its routes and wallet controls are omitted from production bundles. A production build rejects `VITE_CONFIDENTIAL_RFQ_LAB` configuration. The public `/rfq` page integrates the mainnet swap form, browser wallet adapter and encrypted quote-room transport. Native Ready acceptance still requires a complete run with the wallet unlocked. Earlier RFQ URLs redirect to this same workspace. Stop the command to delete its disposable keys, state and devnet; this is not persistent wallet storage.
...
Amounts, asset addresses and destinations are private inputs to the settlement proof. Escrow deployment/activity, public ephemeral signing keys, deadline, timing, nullifiers, ciphertext/proof sizes and fees remain observable. Public shielding/unshielding and external hedges reveal their own amounts and assets. The peer knows this trade; it can disclose it. Deploying the escrow from a personal wallet can reveal a link to that wallet. An external proving service can read the witness. The accepted real-proof settlement does not establish secrecy from the hosted prover or traffic analysis resistance; the local development environment still simulates proving.
...
Agreements, prepared operations and the shared viewing material are sensitive. Exchange them only through an authenticated encrypted channel and retain them in each wallet's secure storage. The core SDK requires an explicit transport adapter; the application integrates encrypted quote rooms, browser signing and durable state. A separate confidential maker implementation handles this protocol; the historical public-inventory bot does not. Native Ready acceptance and independent-market operation have not been established by the controlled SDK settlement.
...
`cairo/src/confidential_escrow.cairo` is part of the normal Cairo build. `src/lib/confidential-rfq-deployment.ts` pins both Sierra and CASM hashes. The check script refuses a mismatch without rewriting the pins. CI's real-pool suite runs the SDK integration test; release checks require the activation and real-proof verification settings to match the recorded mainnet evidence. `realProofVerified` is true for the accepted controlled settlement; `nativeReadyEndToEndVerified` remains false. Generated media and proof artifacts are ignored under `artifacts/`. The local submission manifest preserves the three earlier verified swaps and appends the verified Chat and confidential settlement hashes as post-deadline evidence; the published deadline release is unchanged.


## Verification recorded for this implementation

September 8, 2026: [settlement transaction 0x59435ee6…](https://starkscan.co/tx/0x59435ee65a7f42ed41b329c2df8bb9be7cf5fdfe69ddfd8cb2d08230412696) succeeded at block 14530706 and was accepted on L2 at verification. It applied one proof with two spent-note events, two encrypted output events and the exact settlement callback. Fixed-block discovery before and after confirmed both agreed recipient increases, an empty settled escrow and unchanged historical OPEN balances. Setup and both encrypted funding transactions are recorded in the [sanitized evidence](../deployments/mainnet/confidential-settlement-2026-09-08.json); private terms, keys and exact balances are excluded. This proves the controlled SDK settlement, not independently owned wallets, native Ready acceptance, mainnet refund execution or independent review.
...
The historical version-8 demo is 136 seconds / 4,080 frames at 1080p30. Its one-sided Chat offer acceptance and public maker onboarding predate the private-only policy and must be recaptured before publication as the current product. Its delivered audio/video decodes cleanly, and all 12 spoken endings match their source waveforms with at least 1.5 seconds of space after narration. [Demo notes](HACKATHON_DEMO.md). Those September 7 development and media checks performed no public deployment or new mainnet transaction; the separate September 8 chain evidence above supersedes their real-proof limitation.
