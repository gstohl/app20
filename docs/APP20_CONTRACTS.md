...
- **Legacy role:** enum variants `Fund` (0), `Fill` (1), `Claim` (2), and `Timeout` (3) remain ABI-compatible in source, tests, and recovery UI. The v3 variants are `Lock` (4), `Take` (5), `SettleProceeds` (6), and `ReleaseCollateral` (7). This release does not remove variants 0–3; any future compatibility change would require explicit versioning and migration, never silent reinterpretation.
...
- **Take authorization and public call/events:** the retained `taker_commitment` name stores the ephemeral taker Stark public-key x-coordinate. Cairo verifies `(signature_r, signature_s)` over the escrow address, deal id, ordered token pair, and Poseidon `fills_digest` of ordered `(lock_id, amount_a)` pairs; the private signing key is never calldata. The current pool helper does not expose output-note identity to this message, so a relayer/sequencer with the signed Take can copy it and race with another note id. V3 emits `LockCreated`, `LockTaken`, `DealTaken`, `LockProceedsSettled`, and `LockCollateralReleased`. `LockCreated` publishes the schedule, token pair, RFQ id, expiry, maximum B, ticket, and taker public key. Each `LockTaken` publishes exact per-lock A/B amounts and remaining B; `DealTaken` publishes exact aggregate A/B totals, fill count, and mandatory ordered `fills_digest`.
