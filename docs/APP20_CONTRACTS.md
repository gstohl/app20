...
- **Role:** accepts pool-routed `privacy_invoke` / `privacy_invoke_with_computation`, emits encrypted Mail payloads, keeps a public mailbox-key directory, enforces optional identity-bound action replay protection, and can return one fixed-size recovery deposit.
...
Mail recovery funding is also transaction-local. Before a Mail batch withdraws the fixed seven-unit recovery amount to the helper, the outer account calls `prepare_funding(token)` in the same transaction. The helper consumes that token snapshot once and returns a recovery deposit only when the new balance delta is exactly seven; old donations are excluded, while short or excess deltas revert. A Mail call without a current snapshot remains message-only and returns no recovery deposit, even if the helper already holds that token.

This is exercised by the repository's pinned localnet pool client through a local `compute_and_invoke` action shim. The published Wallet API 0.10 TypeScript action union does not currently declare that variant, so this implementation is not a claim of general production-wallet support and does not change the public-network deployment prohibition above.

### `App20Escrow`

- **Source:** `cairo/src/escrow.cairo`
- **Constructor:** `(pool, ticket_class_hash, lock_ticket_class_hash)`. The third argument changes constructor calldata and requires a fresh deployment; it is not an in-place upgrade.
...
- **Legacy role:** enum discriminants remain `Fund` (0), `Fill` (1), `Claim` (2), and `Timeout` (3), followed by `Lock` (4), `Take` (5), `SettleProceeds` (6), and `ReleaseCollateral` (7). Retaining those discriminants does not make this release wire-compatible: `FundParams` now encodes an explicit `amount`, the protected nonzero Mail and Take call paths changed, and Take uses the v4 signed message. Variants 0–3 remain in source, tests, and recovery UI, but callers must use the matching current ABI and builders.
- **Views:** `ensure_lock_ticket`, `get_lock_ticket`, `get_lock`, `get_take`, and `quote_schedule` are additive. Schedule evaluation accepts one to four positive u128 points, requires increasing A/non-decreasing B, and floors interpolation. `quote_schedule` rejects expired and closed locks; the pure evaluator remains available for historical calculations.
- **State:** the on-chain lock is `Open` while either settlement side remains unfinished and becomes `Closed` after both ticket-authorized sides are finalized. `remaining_b`, `earned_a`, `proceeds_settled`, and `collateral_released` carry its progress.
- **Input accounting:** `FundParams` carries an explicit amount. Before each Fund, Fill, Lock, or Take pool transfer, the outer account transaction calls public `prepare_funding(token)`. The escrow snapshots its actual balance and the native transaction hash, consumes that snapshot once, and accepts only the exact new delta required by the operation. Old donations are excluded and excess input fails. The local wallet and maker insert these calls with `scripts/escrow-funding-preflight.mjs` and reject more than one funded operation for the same token in a batch.
...
- **Take authorization and public call/events:** the retained `taker_commitment` name stores the ephemeral taker Stark public-key x-coordinate. Through the protected ComputeAndInvoke path, Cairo derives the public commitment `H('app20-take-id-v1', private pool identity key, deal id)` without exposing the raw identity key. It verifies `(signature_r, signature_s)` over the `app20-take-v4` domain, native chain ID, escrow address, identity commitment, deal id, ordered token pair, and Poseidon `fills_digest` of ordered `(lock_id, amount_a)` pairs. This binds execution to the pool identity that owns the output note. The commitment differs across RFQs; the signature, fills, and events still link activity within the same RFQ. The pinned local client shim is not declared by the published Wallet API 0.10 action union and does not establish production-wallet compatibility. V3 emits `LockCreated`, `LockTaken`, `DealTaken`, `LockProceedsSettled`, and `LockCollateralReleased`. `LockCreated` publishes the schedule, token pair, RFQ id, expiry, maximum B, ticket, and taker public key. Each `LockTaken` publishes exact per-lock A/B amounts and remaining B; `DealTaken` publishes exact aggregate A/B totals, fill count, and mandatory ordered `fills_digest`.
...
**Coordinated localnet rollout:** deploy the current Mail helper, escrow, tickets, and compatible client/builders together on a fresh localnet. Existing deployed contracts and private notes are not upgraded or migrated by these repository changes; preserve any old runtime/history as historical evidence. This local coordination grants no authority to deploy on a public network.

### `LockTicket` (localnet v3 only)

- **Source:** `cairo/src/lock_ticket.cairo`
- **Constructor:** escrow address, pool address, and lock id.
- **Role:** lock-unique, zero-decimal, supply-two authorization token. `Lock` mints two units into escrow and approves the pool for the two-unit OPEN note. After expiry, each unit can be withdrawn back to escrow and burned to authorize one maker finalization: proceeds or unused collateral. A zero-valued side consumes its unit without creating an unfunded output note.
