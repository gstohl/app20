...
- concurrent reservation serialization, signature equivocation, WAL tampering, truncated-tail recovery, PID locking, durable `reconcile-pending`/`settlement-unknown` recovery, malformed-evidence quarantine, and SIGKILL recovery;
...
| Maker        | CAS fence port, authenticated operation requirement, durable unresolved outcome states with chain-only resolution/backoff, malformed-evidence quarantine, localnet WAL crash tests                                                         | No replicated ledger, HSM custody or independent administration                                                                                      |
