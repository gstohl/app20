...
Exact size + local floor
  ...
  → one protected pool ComputeAndInvoke(Take) atomically deposits total A and returns total B
...
`Take` is all-or-nothing. Every lock must be open, unexpired, tied to the same RFQ and taker public key, and able to cover its evaluated payout. The pinned localnet pool's protected ComputeAndInvoke path derives `H('app20-take-id-v1', private pool identity key, RFQ/deal id)` inside the helper; the raw identity key is never exposed. Cairo verifies an ephemeral Stark signature over the native chain ID, escrow address, this public identity commitment, RFQ/deal id, ordered tokens, and mandatory ordered `fillsDigest`. Any failed fill reverts the transaction. A successful Take creates one OPEN note for the aggregate token B output; there is no taker claim ticket, funded waiting state, maker fill call, or taker timeout/refund in v3. The private signing key is never calldata and is deleted after a terminal Take. The public commitment differs across RFQs, while the signature, fills, and events still link activity within the same RFQ. ComputeAndInvoke is implemented through a pinned local client shim that is not declared by the published Wallet API 0.10 action union, so this is local evidence rather than production-wallet support.

Every value-input operation uses a public `prepare_funding(token)` call before the pool transfer in the same outer account transaction. The escrow records the actual transaction hash and pre-transfer balance, consumes that snapshot once, and requires the new balance delta to equal the operation's explicit or derived amount. Mail uses the same transaction-local pattern for its fixed seven-unit recovery deposit. Pre-existing donations are excluded, short input cannot borrow unrelated surplus, and excess input reverts. Mail without a current snapshot remains message-only and cannot recover ambient helper funds. The local wallet and maker share `scripts/escrow-funding-preflight.mjs`, which rejects more than one funded operation for the same helper and token in a batch.

## Collateral and maker recovery

`App20Escrow` mints two units of a lock-unique `LockTicket` when token B collateral is locked. The two units become the maker's OPEN ticket note. At or after expiry, one unit authorizes `SettleProceeds` for earned token A and one authorizes `ReleaseCollateral` for unused token B. A zero side still consumes its unit but omits the unfunded OPEN output note. Maker nodes scan every five seconds, read `get_lock`, finalize each unset side, and persist transaction attempts and hashes in their hash-chained WAL. After both units are consumed the Cairo lock becomes `Closed`; expired locks are not executable quotes. Transient RPC/read failures become durable `reconcile-pending`; uncertain submitted settlement becomes `settlement-unknown`. Neither state is complete or reusable inventory. Reconciliation retries with bounded backoff and only chain evidence can move the record to `taken`, `expired`, or `settled`; contradictory or irrecoverably malformed evidence remains quarantined.
