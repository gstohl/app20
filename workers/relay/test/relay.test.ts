...
import type { AtomicGate, GateAcquireRequest, GateLease, RelayEnv } from "../src/types.ts";
...
class SharedGate implements AtomicGate {
  ...
  async acquire(_input: GateAcquireRequest): Promise<GateLease> {
    if (this.active >= this.max) throw new RelayHttpError(429, "Relay quota exceeded.");
    this.active += 1;
    this.acquisitions += 1;
    let released = false;
    return { release: async () => { if (!released) { released = true; this.active -= 1; } } };
  }
}
...
async function ohttpRequest(cookie: string, init: RequestInit = {}): Promise<Request> {
  ...
  return new Request("https://app.invalid/api/ohttp/prover", {
    ...
    method,
    headers,
    body: method === "GET" || method === "HEAD" || init.body === null ? null : (init.body ?? new TextEncoder().encode(PLAINTEXT_CANARY)),
  });
}

async function sessionCookie(environment = env()): Promise<string> {
  return (await issueOhttpSession("privy-did-wallet-identity-canary", environment, 1_700_000_000_000)).split(";", 1)[0];
}
...
test("forwards and returns opaque OHTTP bytes unchanged with stripped client headers", async () => {
  ...
  const handler = createRelayHandler({
    ...
    fetch: async (input, init) => {
      ...
      assert.equal(headers.get("authorization"), "Bearer AUTHORIZATION_CANARY_NOT_CLIENT_VISIBLE");
      forwarded = new Uint8Array(init?.body as ArrayBuffer);
      return new Response(responseBytes, { headers: { "content-type": "message/ohttp-res" } });
    },
  });
  ...
  try {
    const handler = createRelayHandler({
      ...
      fetch: async (_input, init) => {
        assert.equal(init?.redirect, "error");
        throw new TypeError(`${PLAINTEXT_CANARY} ${SECRET_CANARY} ${UPSTREAM_CANARY}`);
      },
    });
    const response = await handler(await ohttpRequest(cookie), env());
    const output = await response.text();
    assert.equal(response.status, 502);
    for (const canary of [PLAINTEXT_CANARY, SECRET_CANARY, UPSTREAM_CANARY, "AUTHORIZATION_CANARY"]) {
...
test("rejects declared and streamed oversized OHTTP bodies", async () => {
  const cookie = await sessionCookie();
  const handler = createRelayHandler({ now: NOW, gate: new SharedGate(), fetch: async () => { throw new Error("must not fetch"); } });
  const declared = await handler(await ohttpRequest(cookie, { headers: { "content-length": String(2 * 1024 * 1024 + 1) } }), env());
  ...
  const streamedRequest = new Request("https://app.invalid/api/ohttp/prover", {
    method: "POST",
    headers: { origin: "https://app.invalid", "content-type": "message/ohttp-req", cookie },
    body: stream,
    duplex: "half",
...
test("rejects wrong origin, content type, and method", async () => {
  const cookie = await sessionCookie();
  const handler = createRelayHandler({ now: NOW, gate: new SharedGate(), fetch: async () => { throw new Error("must not fetch"); } });
  assert.equal((await handler(await ohttpRequest(cookie, { headers: { origin: "https://evil.invalid" } }), env())).status, 403);
  assert.equal((await handler(await ohttpRequest(cookie, { headers: { "content-type": "application/json" } }), env())).status, 415);
  assert.equal((await handler(await ohttpRequest(cookie, { method: "PUT", body: undefined }), env())).status, 405);
});

test("fails closed without Origin except for explicit loopback development", async () => {
  const cookie = await sessionCookie();
  const gate = new SharedGate();
  const handler = createRelayHandler({ now: NOW, gate, fetch: async () => new Response(new Uint8Array([1]), { headers: { "content-type": "message/ohttp-res" } }) });
  const production = await handler(new Request("https://app.invalid/api/ohttp/prover", { method: "POST", headers: { "content-type": "message/ohttp-req", cookie }, body: "x" }), env());
  assert.equal(production.status, 403);
  const devEnv = env({ ENVIRONMENT: "development", ALLOW_LOCAL_DEVELOPMENT: "true", ALLOW_LOOPBACK_HTTP: "true", PROVER_UPSTREAM_URL: "http://127.0.0.1:8788/relay" });
  const devCookie = await sessionCookie(devEnv);
  const development = await handler(new Request("http://127.0.0.1:8787/api/ohttp/prover", { method: "POST", headers: { "content-type": "message/ohttp-req", cookie: devCookie }, body: "x" }), devEnv);
...
test("two logical relay instances share one concurrency gate and always release", async () => {
  ...
  const fetcher = async () => { entered(); await blocked; return new Response(new Uint8Array([9]), { headers: { "content-type": "message/ohttp-res" } }); };
...
test("RPC rejects unsupported methods and forwards allowed JSON bytes unchanged", async () => {
  ...
  const handler = createRelayHandler({
    ...
    fetch: async (_input, init) => {
      ...
      return new Response('{"jsonrpc":"2.0","result":"0x1","id":1}', { headers: { "content-type": "application/json" } });
    },
  });
  const rpc = (body: string) => new Request("https://app.invalid/api/starknet/sepolia", { method: "POST", headers: { origin: "https://app.invalid", "content-type": "application/json", "x-secret": SECRET_CANARY }, body });
  const unsupported = await handler(rpc('{"jsonrpc":"2.0","method":"eth_sendRawTransaction","id":1}'), env());
  assert.equal(unsupported.status, 400);
  assert.equal((await unsupported.text()).includes("eth_sendRawTransaction"), false);
  const allowedBody = '{"jsonrpc":"2.0","method":"starknet_chainId","params":[],"id":"OPAQUE_RPC_CANARY"}';
