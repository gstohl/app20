...
pub trait IClaimTicket<TState> {
    ...
    fn deal_id(self: @TState) -> felt252;
}

/// Deal-unique, supply-one authorization token. The escrow mints it into the funding OPEN note;
/// later possession of that private note authorizes exactly one claim or timeout.
#[starknet::contract]
pub mod ClaimTicket {
    ...
    impl ClaimTicketImpl of super::IClaimTicket<ContractState> {
        ...
        fn mint(ref self: ContractState) {
            ...
            self.supply.write(1);
            self.balances.entry(escrow).write(1);
            self.emit(Transfer { from: 0.try_into().unwrap(), to: escrow, value: 1 });
        }

        fn burn(ref self: ContractState) {
            let escrow = self.escrow.read();
            assert(get_caller_address() == escrow, errors::NOT_AUTHORIZED);
            assert(self.balances.entry(escrow).read() == 1, errors::BAD_AMOUNT);
            self.balances.entry(escrow).write(0);
            self.supply.write(0);
        ...
        fn deal_id(self: @ContractState) -> felt252 {
            self.deal_id.read()
        }
    }
