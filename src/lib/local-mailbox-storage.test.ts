...
describe("local mailbox purge", () => {
  it("removes every sensitive mailbox store but preserves preferences", () => {
    ...
    storage.setItem("app20/theme", "dark");
    storage.setItem("app20/localnet-wallet/identity/v1", "alice");
    ...
    expect(storage.getItem("app20/theme")).toBe("dark");
    expect(storage.getItem("app20/localnet-wallet/identity/v1")).toBe(
      "alice",
    );
