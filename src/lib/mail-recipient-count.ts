...
const MULTI_RECIPIENT_MARKER = [0x41, 0x32, 0x30] as const;
const MULTI_RECIPIENT_COUNT_OFFSET = 4;

/**
 * Returns the recipient count disclosed by the ciphertext format. Compact
...
export function publicRecipientCount(record: EncryptedMailRecord): number {
  ...
  try {
    ...
    const minimumWireBytes = 5 + count * MULTI_RECIPIENT_SLOT_BYTES + 12 + 16;
    if (
      hasMarker &&
    ...
    // Public event data is untrusted; use the conservative disclosure below.
  }
