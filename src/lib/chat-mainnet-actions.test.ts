...
const input = {
  helperAddress: "0x123", tokenAddress: "0x456", actionId: computeActionId("composite-document", "0x789"),
...
describe("mainnet message-only candidate", () => {
  it("binds the message to the proof without withdrawing funds or opening a recovery note", () => {
    ...
    expect(actions[0]).toEqual({
      ...
      compute_calldata: ["0x456", "0x0", "0x11", "0x22", "0x3", "0x44", "0x55", "0x1", "0x66", input.actionId],
      invoke_calldata: ["0x456", POOL_ADDRESS_PLACEHOLDER, "0x0", "0x11", "0x22", "0x3", "0x44", "0x55", "0x1", "0x66", input.actionId],
    });
