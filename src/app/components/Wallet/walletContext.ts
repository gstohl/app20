...
export type ConnectedWalletSnapshot = Readonly<{
...
export type AccountSwitchSnapshot = Readonly<{
    address: string;
    chain: string;
}>;
...
export interface WalletState {
    ...
    applyConnectedWallet: (snapshot: ConnectedWalletSnapshot) => void;
    applyAccountSwitch: (input: AccountSwitchSnapshot) => void;
...
export const useStoreWallet = create<WalletState>()((set) => ({
    ...
    setSelectWalletUI: (displaySelectWalletUI) =>
    ...
    applyConnectedWallet: ({
        ...
        set({
            ...
            address,
            chain,
            walletApiList: capability.walletApiVersions,
            strk20Capability: capability,
            isStrk20Capable: capability.supported,
            connectionNotice: "",
            isConnected: true,
        }),
    applyAccountSwitch: ({ address, chain }) =>
        set((state) =>
            state.address === address && state.chain === chain
                ? state
                : { address, chain },
        ),
    disconnect: (notice = "") =>
