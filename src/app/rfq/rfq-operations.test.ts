...
describe("browser-safe localnet RFQ operations", () => {
  it("keeps every invited maker with deterministic eligible and excluded rationale", () => {
    ...
    expect(
      status.makers.map((maker) => [
        maker.makerId,
        maker.invitationStatus,
        maker.capacityBand,
        maker.eligible,
      ]),
    ).toEqual([
    ...
    expect(JSON.stringify(status)).not.toMatch(
      /settlementAccount|availableBaseUnits|rawBalance|processPid|operationLog/i,
    );
    expect(status.rawInventoryExposed).toBe(false);
  });

  it.each(["paused", "drain-only"] as const)(
    "blocks starts in %s while preserving recovery",
    (mode) => {
  ...
  it("fails closed on unknown and stale status without disabling claim or refund", () => {
    ...
    const stale = operationsAvailability(
      normalizeRfqOperationsStatus(wire()),
      NOW + 30,
    );
  ...
  it("blocks starts when no maker or the selected maker is eligible", () => {
    const status = normalizeRfqOperationsStatus(
      wire({
        ...
        makers: (wire().makers as Array<Record<string, unknown>>).map(
          (maker) => ({
            ...maker,
    ...
    expect(
      gateRfqAction(
        operationsAvailability(normalizeRfqOperationsStatus(wire()), NOW + 1),
        "fund",
        "missing-maker",
      ).allowed,
    ).toBe(false);
  ...
  it("derives fresh, expiring, and expired directory states from the published deadline", () => {
    ...
    expect(directoryFreshnessState(status.directory, NOW + 20)).toBe(
      "expiring",
    );
  ...
  it("rejects missing, malformed, contradictory, and unknown cohort or key fields", () => {
    ...
    expect(() => normalizeRfqOperationsStatus(missingCohort)).toThrow(
      /schema/i,
    );

    const missingKeyStatus = wire();
    delete (
      (missingKeyStatus.makers as Array<Record<string, unknown>>)[0] as Record<
        string,
        unknown
      >
    ).keyStatus;
    expect(() => normalizeRfqOperationsStatus(missingKeyStatus)).toThrow(
      /schema/i,
    );
    ...
    delete (
      (
        missingKeyDeadline.makers as Array<Record<string, unknown>>
      )[0] as Record<string, unknown>
    ).keyValidUntil;
    expect(() => normalizeRfqOperationsStatus(missingKeyDeadline)).toThrow(
      /schema/i,
    );
      ...
      normalizeRfqOperationsStatus(
        wire({
          makers: (wire().makers as Array<Record<string, unknown>>).map(
            (maker, index) =>
              index === 0
                ? { ...maker, keyStatus: "rotated", eligible: true }
                : maker,
          ),
        }),
      ),
  ...
  it.each(["expired", "rotated"] as const)(
    ...
    (keyStatus) => {
      ...
      expect(
        gateRfqAction(
          operationsAvailability(status, NOW + 1),
          "fund",
  ...
  it("publishes the operations clock only when gating would change", () => {
    const status = normalizeRfqOperationsStatus(wire());
    expect(shouldPublishOperationsClock(status, NOW + 1, NOW + 1)).toBe(false);
  ...
  it("rejects raw health, PID, account, log, inventory, or balance fields", () => {
    for (const forbidden of [
      ...
      expect(() =>
        normalizeRfqOperationsStatus({ ...wire(), ...forbidden }),
      ).toThrow(/forbidden field|schema/i);
    }
  });
});
...
describe("named localnet economic policy", () => {
  it("replaces Instant minBuy=1 with an exact reference/deviation floor in both directions", () => {
    const forward = localnetEconomicReview({
      pairId: "STRK_USDC",
      sellAmount: 10n ** 17n,
      surface: "swap",
    });
    ...
    const reverse = localnetEconomicReview({
      pairId: "USDC_STRK",
      sellAmount: 100_000n,
      surface: "swap",
    });
  ...
  it("enforces reviewed floor deviation and named per-trade caps", () => {
    ...
    expect(() =>
      localnetEconomicReview({
        pairId: "STRK_USDC",
        sellAmount: 51n * 10n ** 18n,
        surface: "swap",
      }),
    ).toThrow(/per-trade cap/i);
