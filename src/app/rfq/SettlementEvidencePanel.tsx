...
export default function SettlementEvidencePanel({ receipt, transactionHashes = [], records = [] }: {
  ...
  return <section className={styles.evidencePanel} aria-labelledby="evidence-title">
    <strong>LOCALNET DEMO EVIDENCE</strong>
    <h3 id="evidence-title">{heading}</h3>
    <p>{reason}</p>
    <p>This authority is localnet-only and modeled by same-devnet fixture readers. Sepolia/Mainnet production authority remains unavailable.</p>
    {unresolved.length ? <div className={styles.evidenceUnresolved}>
      <h4>Needs reconciliation</h4>
      {unresolved.map(({ row, presentation }) => <div key={row.rfqId}><code>{row.rfqId}</code><AuthorityStrip presentation={presentation}/></div>)}
      <p>These records cannot drive a value action until they are reconciled. Nothing is resubmitted for you.</p>
    </div> : null}
    <details><summary>Non-authoritative transaction references</summary>{transactionHashes.length ? transactionHashes.map((hash) => <code key={hash}>{hash}</code>) : <p>None observed.</p>}</details>
    <p>Pending, stale, reorg-invalidated, or reader-disagreement evidence never enables execution.</p>
  </section>;
}
