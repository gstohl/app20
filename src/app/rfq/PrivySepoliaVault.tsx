...
import { PRIVY_APP_ID, PRIVY_CLIENT_ID } from "@/app/rfq/privy-config";
import { assertPrivyRailSelected, useWalletMode } from "@/app/rfq/walletMode";
import {
  BrowserStrk20Client,
...
type PrivateBalances = Awaited<ReturnType<BrowserStrk20Session["balances"]>>;
...
function decodePinnedKey(value: string | undefined, label: string): Uint8Array {
  if (!value)
    throw new Error(`Missing reviewed ${label} OHTTP key configuration.`);
  try {
...
function safeError(error: unknown): string {
  ...
  if (/not deployed/i.test(message))
    return "Deploy the Starknet account first.";
...
function PrivySepoliaVaultContent() {
  ...
  const setPrivyStatus = useWalletMode((state) => state.setPrivyStatus);
  ...
  useEffect(() => {
    ...
    if (
      selectedId &&
      wallets.some((wallet) => wallet.walletId === selectedId)
    ) {
      return;
    }
    const preferred =
      ...
      wallets.find((wallet) => wallet.authorization.browserSignable) ??
      wallets[0];
  ...
  async function signHash(wallet: BootstrapWallet, hash: `0x${string}`) {
    assertPrivyRailSelected(useWalletMode.getState().mode);
  ...
  async function scanPrivate(announce = true) {
    ...
    try {
      ...
      setWalletPrivateState(selected.walletId, {
        status: "error",
        error: message,
      });
      setError(message);
  ...
  async function runAction(
    ...
    if (!selected.authorization.browserSignable) {
      setError(
        "The current Privy signer cannot satisfy this wallet quorum alone.",
      );
      return;
    }
  ...
  if (!authenticated) {
    return (
      <section
        ...
        aria-labelledby="privy-login-title"
      >
  ...
  const notes =
    currentPrivate?.balances?.flatMap((balance) =>
    ...
    <div className={styles.layout}>
            ...
            return (
              ...
              >
                ...
                <span className={styles.accountIdentity}>
                  ...
                  <code className={styles.accountAddress}>
                    {short(address)}
                  </code>
      ...
      >
        ...
        <section
          ...
          aria-labelledby="privy-account-title"
        >
          <header className={styles.sectionHeader}>
          ...
          <dl className={styles.facts}>
            <div className={styles.fact}>
              <dt>DEPLOYMENT</dt>
              <dd>
                {accountQuery.data?.deployed ? "DEPLOYED" : "COUNTERFACTUAL"}
              </dd>
            </div>
            <div className={styles.fact}>
              <dt>QUORUM</dt>
          ...
          {buildOnly ? (
            <div
              className={`${styles.warning} ${styles.buildWarning}`}
              role="status"
            >
              <strong>BUILD-ONLY SAFETY MODE</strong>
              <span>
                Proofs and calls can be reviewed. No private transaction will be
                submitted from this rail.
              </span>
            </div>
          ) : null}
        ...
        <section
          ...
          aria-labelledby="recovered-notes-title"
        >
          <header className={styles.sectionHeader}>
            <div>
              ...
              <strong
                ...
                id="recovered-notes-title"
              >
                Recovered notes
              </strong>
            </div>
            <span className={styles.sectionMeta}>
              {currentPrivate?.status === "scanning"
                ? "SCANNING"
                : `${notes.length} LOCAL`}
            </span>
          </header>
          {notes.length ? (
            <div
              ...
              role="table"
              aria-label="Recovered private notes"
            >
              ...
              {notes.map((note, index) => (
                ...
                >
                  ...
                  <span
                    className={
                      note.mature ? styles.matureState : styles.waitingState
                    }
                    role="cell"
                  >
            ...
            <div className={styles.emptyNotes}>
              <span className={styles.emptyNoteMark} aria-hidden="true">
                ◇
              </span>
              <div>
      ...
      <aside
        className={`${styles.panel} ${styles.statusRail}`}
        aria-label="Privy trust state"
      >
        <header className={styles.sectionHeader}>
          <div>
        ...
        <dl className={styles.trustList}>
          ...
          <div className={styles.trustRow}>
            <dt>REMOTE PROVER</dt>
            <dd className={styles.cautionState}>SEES WITNESS</dd>
          </div>
          <div className={styles.trustRow}>
            <dt>READY FALLBACK</dt>
            <dd>DISABLED</dd>
          </div>
        </dl>
        ...
        <section
          ...
          aria-labelledby="privy-activity-title"
        >
...
export default function PrivySepoliaVault() {
  if (!PRIVY_APP_ID || !PRIVY_CLIENT_ID) {
    return (
      <section
        className={styles.unconfiguredPanel}
        aria-labelledby="privy-unconfigured-title"
      >
        ...
        <dl className={styles.configChecklist}>
          <div>
            ...
            <dd
              className={
                PRIVY_APP_ID ? styles.configReady : styles.configMissing
              }
            >
          ...
          <div>
            ...
            <dd
              className={
                PRIVY_CLIENT_ID ? styles.configReady : styles.configMissing
              }
            >
