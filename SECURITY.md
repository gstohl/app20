...
- `takerCommitment` is the retained wire name for an ephemeral Stark public key, not a secret or hash. `LockCreated` publicly reveals that key plus the RFQ id, pair, expiry, maximum collateral, complete schedule, and ticket. `LockTaken` reveals exact per-lock A/B amounts and remaining collateral. `DealTaken` reveals exact aggregate A/B totals, fill count, and ordered `fillsDigest`; Take calldata exposes the signature and ordered fills. The signature binds escrow, deal, token ordering, and fills, but the current pool API does not let it bind output-note ownership. A relayer or sequencer with the signed Take can therefore copy it and race with a different output note; first accepted Take wins. Shield/unshield boundaries, OPEN-note amounts, helper use, and timing are also public and correlatable.
...
- Contact/RFQ backups are encrypted in the browser. RFQ restore authenticates candidates before sequence ranking, rejects same-sequence equivocation and rollback, carries bounded authenticated deletion tombstones, and permanently marks restored v3 rows verify-only without signing keys. A fresh device given only an older authentic snapshot that predates a deletion cannot infer the absent newer tombstone. An IPFS RPC or gateway still learns source network metadata, timing, requested CID, and padded ciphertext size; a pinning service stores the encrypted bytes. The localnet IPFS emulator is loopback-only, in-memory, and loses all blocks on restart.
...
- Public-network RFQ and value execution remain immutable-off. The mounted localnet desk exercises v3 and keeps legacy lifecycle recovery available, but same-devnet implementation and browser tests are not public deployment evidence.
