...
function eventFromRpc(value) {
  return Object.freeze({
    ...
    keys: Object.freeze((value?.keys ?? []).map((item, index) => canonicalEventKey(item, `event key ${index}`))),
    data: Object.freeze((value?.data ?? []).map((item, index) => {
...
export function createLocalnetJsonRpc(url, options = {}) {
  ...
  return async function rpc(method, params) {
    ...
    if (!response.ok || payload?.error || payload?.result === undefined)
      throw new Error("Localnet authority RPC observation is unavailable.");
...
export function createLocalnetRpcReader(options) {
  ...
  if (typeof rpc !== "function") throw new Error("Localnet RPC reader requires an injected RPC adapter.");
  ...
  return Object.freeze({
    ...
    async observe(query) {
      ...
      if (classHash !== artifact.escrowClassHash)
        throw new Error("Localnet RPC reader observed another escrow class hash.");
      const stages = query.outcome === "settled"
        ? ["fund", "fill", "claim"]
        : ["fund", "timeout"];
      const lifecycle = [];
      for (const stage of stages) {
        ...
        if (receipt?.execution_status && receipt.execution_status !== "SUCCEEDED")
          throw new Error("Localnet authority refuses a reverted lifecycle transaction.");
        const receiptHash = canonicalFelt(receipt.transaction_hash, "receipt transaction hash");
        if (receiptHash !== transactionHash)
          throw new Error("Localnet authority receipt hash was substituted.");
        const number = blockNumber(receipt.block_number, "receipt block number");
        const hash = canonicalFelt(receipt.block_hash, "receipt block hash");
        const block = await rpc("starknet_getBlockWithTxHashes", {
          block_id: { block_number: number },
        });
        const transactions = (block.transactions ?? []).map((item, index) =>
          canonicalFelt(typeof item === "string" ? item : item?.transaction_hash, `block transaction ${index}`),
        );
        const transactionIndex = transactions.indexOf(transactionHash);
        if (transactionIndex < 0)
          throw new Error("Receipt transaction is absent from its block number.");
        ...
        if (matches.length !== 1)
          throw new Error("Lifecycle transaction does not contain one exact pinned escrow event.");
        lifecycle.push(Object.freeze({
          ...
          block: Object.freeze({
            number: blockNumber(block.block_number, "canonical block number"),
            hash: canonicalFelt(block.block_hash, "canonical block hash"),
